...
6. **中 / 英切换**：右上角 `中文 / EN` 按钮。已生成的语言瞬时切换;首次切到另一语言会用当前 LLM 翻译一次(约几秒~1 分钟，取决于引擎)，之后缓存。软件/编程语言/品牌名在两种语言下都保持英文。
7. **导出**：右上「导出 PNG / 导出 PDF」，纯前端生成，文件名取自姓名。
8. **顶部状态条**会显示本次实际使用的解析引擎（如 `ollama` / `anthropic` / `mock`），便于确认是否走了 LLM。

> 切换语言需要配置了 LLM（Claude / OpenAI / Ollama）才能真正翻译；`mock` 模式下仅切换界面文案、正文内容不变。
